...
data_post_1995_utm10km %>%
  filter(spec_name == "Anodonta anatina") %>%
  full_join(utm10_sf, by = join_by(utm_10km == TAG)) %>%
  st_as_sf() %>%
  ggplot() +
    geom_sf(data = belgium_sf, fill = "white",
            linewidth = 0.8) +
    geom_sf(data = waterlopen_sf, aes(colour = waterloop),
            linewidth = 0.8) +
    scale_color_manual(values = c("cornflowerblue", "darkblue")) +
    geom_sf(aes(fill = state_fill), linewidth = 0.6, alpha = 0.7) +
    scale_fill_discrete(na.translate = FALSE) +
    labs(title = "Anodonta anatina", x = "", y = "",
         fill = "Status", colour = "Waterloop") +
    coord_sf(datum = proj_crs) +
    theme_void() +
    theme(legend.position = c(0.2, 0.2),
...
data_post_1995_utm10km %>%
  ...
  ggplot() +
    ...
    geom_sf(data = waterlopen_sf, aes(colour = waterloop),
            linewidth = 0.8) +
    scale_color_manual(values = c("cornflowerblue", "darkblue")) +
    geom_sf(aes(fill = state_fill, pattern = state_fill),
